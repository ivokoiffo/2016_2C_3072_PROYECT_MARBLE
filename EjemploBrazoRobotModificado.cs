...
namespace TGC.Examples.Transformations
{
    public class EjemploBrazoRobot : TGCExampleViewer
    {
            ...
            : base(mediaDir, shadersDir, userVars, modifiers)
        {
            ...
            Name = "Brazo de robot";
            Description =
                "Ejemplo brazo de robot. ARR/ABA->ang brazo, Der/Izq->ang antebrazo, A/S -> ang pinza, Q/W -> abrir , cerrar pinza T/Y=Rotacion en X";
        }
        ...
        public override void Update()
        {
            ...
            if (Input.keyDown(Key.A))
            {
                ...
                if (pinzaDang >1){ pinzaDang = 1; };
            }
            else if (Input.keyDown(Key.S))
            {
                pinzaDang -=VELOCIDAD_ANGULAR * ElapsedTime;
                if (pinzaDang <-0.5f) { pinzaDang = -0.5f; };
            }

            if (Input.keyDown(Key.Q))
            {
                pinzaTraslacion += VELOCIDAD_ANGULAR * ElapsedTime;
                if (pinzaTraslacion > 0.3f) { pinzaTraslacion = 0.3f; };
            }
            else if (Input.keyDown(Key.W))
            {
                pinzaTraslacion -= VELOCIDAD_ANGULAR * ElapsedTime;
                if (pinzaTraslacion < 0f) { pinzaTraslacion = 0f; };
            }
            ...
            if (Input.keyDown(Key.Up))
            {
                ...
                if (brazoDAng > 0.5f) { brazoDAng = 0.5f; };
            }
            ...
            {
                brazoDAngX += VELOCIDAD_ANGULAR * ElapsedTime;
                if (brazoDAngX > 0.5f) { brazoDAngX = 0.5f; };
            }
            ...
            var Rot2 = Matrix.RotationX(brazoDAngX);
            ...
            transformacionBrazo = T * A * Rot* Rot2 * B;
